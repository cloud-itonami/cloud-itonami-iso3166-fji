  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Suva Fabrication Pte Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Suva Fabrication Pte Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest simplified-procurement-ineligible-is-held-and-unoverridable
  (testing "declared :claims-simplified-procurement? true but the declared contract value exceeds FJD 50,000 -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:simplified-procurement-ineligible} (-> (store/ledger db) last :basis)))
...
(deftest frcs-tin-record-unverified-is-held-and-unoverridable
  (testing "unverified FRCS TIN record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:frcs-tin-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
