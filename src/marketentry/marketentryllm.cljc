(ns marketentry.marketentryllm
  ...
  the Republic of Fiji (FJI) public-sector market-entry compliance
  actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "claims-simplified-procurement?=" (:claims-simplified-procurement? e)
                        " declared-contract-value=" (:declared-contract-value e)
                        " frcs-tin-record-verified?=" (:frcs-tin-record-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-frcs-tin-record? e))
                              (:frcs-tin-record-verified? e)))
                   0.9 0.3)}))
