...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (FJI, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up simplified-procurement-ineligible) ==")
    ...
    (println "== filing/submit eng-4 (simplified-procurement-ineligible -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up frcs-tin-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (frcs-tin-record-unverified -> HARD hold) ==")
